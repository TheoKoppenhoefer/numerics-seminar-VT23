...
\begin{frame}
	...
	\begin{tikzpicture}[remember picture, overlay, node distance = 0.3cm]
		\node[,text width=10cm] (def_gamma_descr) [above right=0.2cm and 0.1cm of def_gamma]{$\gamma=\brk*{Y_k^\top Y_k}^{-1}Y_k^\top$};
...
%	\IncMargin{1em}
	...
	\For{$k=0,1,\dots$}{
		...
		{\color{black}Construct $S_k$ and $Y_k$ from $g_0,\dots,g_k$ and $x_0,\dots, x_k$.
		
		Set $H_k = \Id +(S_k-Y_k)(Y_k^\top Y_k)^{-1}Y_k^\top$.
...
\begin{frame}
	...
	\end{proposition}
	From Broydens method we know that it is a good idea to approximate the Jacobian rather than its inverse. This yields the definition of $B_k$ which minimises $\norm{B_k-\Id}_F$ under the condition $B_kY_k=S_k$. Analogously to AA-II we have
	\begin{align*}
...
%	\IncMargin{1em}
	...
	\For{$k=0,1,\dots$}{
		...
		Construct $S_k$ and $Y_k$ from $g_0,\dots,g_k$ and $x_0,\dots, x_k$.
